...
import { Users, Database, FolderOpen, Shield, Bot, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Sidebar({ agents, connections, isConnected }: SidebarProps) {
  ...
  return (
    <div className="relative flex">
      ...
      >
        ...
        <div className="p-4 border-b border-border">
          <div className="flex items-center justify-center w-full">
            {isOpen ? (
              <img src={scodacLogo} alt="SCODAC" className="h-12 w-auto transition-all duration-300" />
            ) : (
              <img src={scodacFavicon} alt="SCODAC" className="h-12 w-12 transition-all duration-300" />
            )}
          </div>
        </div>
        ...
        <div className="flex-1 p-4">
          {isOpen && <h3 className="text-sm font-semibold text-foreground mb-4">Agent Status</h3>}
          <div className={`${isOpen ? "space-y-3" : "space-y-4"}`}>
            {agents.map((agent) => {
              const IconComponent = agentIcons[agent.type as keyof typeof agentIcons] || Bot;
              const colorClass = agentColors[agent.type as keyof typeof agentColors] || "bg-muted";
              const statusColor = statusColors[agent.status as keyof typeof statusColors] || "bg-muted";
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      {isOpen && (
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-muted-foreground" data-testid={`text-agent-status-${agent.type}`}>
        ...
        {isOpen && (
          <div className="p-4 border-t border-border">
            <h3 className="text-sm font-semibold text-foreground mb-3">API Connections</h3>
                  ...
                  <div className="flex items-center space-x-1">
                    <div
                      className={`w-2 h-2 rounded-full ${
                        connection.status === "online" ? "bg-green-500" : "bg-destructive"
                      }`}
                    />
                    <span
                      className={`${connection.status === "online" ? "text-green-500" : "text-destructive"}`}
      ...
      <Button
        ...
        className="absolute -right-4 top-6 z-10 h-8 w-8 min-h-8 min-w-8 p-0 bg-background border border-border rounded-full shadow-md hover:bg-muted transition-all duration-300 flex items-center justify-center"
